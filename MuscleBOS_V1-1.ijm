...
// Versopm 1.1: JAP 04-10-2026 adds options for hybrid fiber (Type IIA/IIX)
//
// Authors: J.Alex Pasternak
//
...
function TypeCartographyFillJAP(LamininWindows,NameTypeImage,FiberPhenoType1,FiberPhenoType2amb,FiberPhenoType2a,FiberPhenoType2x,FiberPhenoTypeNot2X,FiberPhenoTypeNA,
	...
	for (i=0 ; i<nbFibers; i++) {
		...
		if (FiberPhenoType2a[i] ==2) {
			setForegroundColor(FILTERS_Hybrid[5], FILTERS_Hybrid[6], FILTERS_Hybrid[7]);
		}
	...
	if (LegendBox=="Yes") {
		...
		setFont("Sanserif", 75);
		setJustification("center");
		if ((checkAnalysis[1]) && (checkAnalysis[3])&& (!checkAnalysis[8]))  {
			Y=Y-650;
		...
		if ((checkAnalysis[1]) && (checkAnalysis[3]) && (checkAnalysis[8]))  {
			...
			setForegroundColor(FILTERS_Hybrid[5],FILTERS_Hybrid[6],FILTERS_Hybrid[7]);
			run("Fill");
			SetCouterCol(FILTERS_Hybrid[5],FILTERS_Hybrid[6],FILTERS_Hybrid[7]);
			drawString("Type IIA/IIX",X+150, Y+offset);
			Y=Y+155;
			makeRectangle(X, Y, 300, 150,25);
			setForegroundColor(FILTERS_IIX[5],FILTERS_IIX[6],FILTERS_IIX[7]);
			run("Fill");
...
checkAnalysis = newArray(0,0,0,0,0,0,0,0,0);
...
while(CHECK<1){
	...
	labels=newArray("Fiber Morphology", "Fiber Type I", "Centro Nuclei", "Fiber Type All but IIX", "Adjust Thresholds", "Filter on Fiber Size", "Set Size Catagories", "Output Signal Intensity","Include Hybrid IIA/IIX Fibers");
	defaults = newArray(1,1,0,1,0,1,0,1,0);
	...
	}else if(checkAnalysis[8] && (!checkAnalysis[1] || !checkAnalysis[3])){
		waitForUser("Hybrid fiber detection is only relevant when using both Type I and All but IIX");
	}else {
		CHECK=1;
	}
...
FILTERS_IIA=newArray(0,10,1000000,1,0.5,CN[3],0,0);
FILTERS_IIX=newArray(1,1000,1000000,1,0,CN[1],0,0);
FILTERS_IIamb=newArray(0,1500,1000000,1,1,CN[11],0,0);
FILTERS_Not2X=newArray(0,10,1000000,1,0.5,CN[8],0,0);
FILTERS_Hybrid=newArray(1,1000,1000000,1,0,CN[6],0,0);
...
while (CHECK<1){
	...
	if(checkAnalysis[3]){
		...
		if(checkAnalysis[4]){
			Dialog.addNumber("Upper Threshold Mean Multiplier",FILTERS_IIA[3],1,2,"");
			Dialog.addToSameRow();
			Dialog.addNumber("SD Multiplier",FILTERS_IIA[4],1,2,"");
			Dialog.addNumber("Lower Threshold Mean Multiplier",FILTERS_IIX[3],1,2,"");
			Dialog.addToSameRow();
			Dialog.addNumber("Lower Threshold SD Multiplier",FILTERS_IIX[4],1,2,"");
		}
		if(checkAnalysis[5]){
			//IIA
			if(checkAnalysis[1]){
			...
			if(checkAnalysis[8]){
				//hybrid
				Dialog.addChoice("Size Filter Type IIA/IIX", YN,"Yes");
				Dialog.addToSameRow();
				Dialog.addNumber("Min Fiber Area", FILTERS_Hybrid[1],0,4,"");
				Dialog.addToSameRow();
				Dialog.addNumber("Max Fiber Area", FILTERS_Hybrid[2],0,8,"");
			}
		}
		if(CUS_COLOUR=="Yes" && (CartographyBox=="Fiber Types"||CartographyBox=="All Relevant")){
			if(checkAnalysis[1]){
				...
				if(checkAnalysis[8]){
					Dialog.addToSameRow();
					Dialog.addChoice("Type IIA/IIX", CN, FILTERS_Hybrid[5]);
				}
			}else{
	...
	if(checkAnalysis[3]){
		...
		if(checkAnalysis[5]){
			...
			if(checkAnalysis[8]){
				FILTERS_Hybrid[0]=Dialog.getChoice();
				FILTERS_Hybrid[1]=Dialog.getNumber();
				FILTERS_Hybrid[2]=Dialog.getNumber();
			}
		}
		if(CUS_COLOUR=="Yes" && (CartographyBox=="Fiber Types"||CartographyBox=="All Relevant")){
			if(checkAnalysis[1]){
				...
				if(checkAnalysis[8]){
					FILTERS_Hybrid[5]=Dialog.getChoice();
				}
			}else{
...
if(FILTERS_Hybrid[0]=="Yes"){FILTERS_Hybrid[0]=1;}else{FILTERS_Hybrid[0]=0;}
...
FILTERS_Hybrid = ColCon(FILTERS_Hybrid);
...
for (ik=0; ik<list.length; ik=ik+NbChannels) {
	...
	if(!filefound) {
		...
		nbBAD5pos=nbBAD5neg=nbBF35pos=nbBF35dim=nbBF35neg=nbFiberTypeHybrid=0;
		AREA_I=AREA_2A=AREA_2X=AREA_NA=AREA_Not2X=AREA_2amb=AREA_Hybrid=0;
		for (j=0; j<nbFibers; j++) {
				...
				if ((FiberPhenoType1[j]==-1) && (FiberPhenoType2a[j]>0)) {
					...
					if (checkAnalysis[8]){
						if(FiberPhenoType2a[j]==1){
							...
							//hybrid fiber
							if(FILTERS_IIA[0]){
								if(FiberArea[j]>FILTERS_Hybrid[1] && FiberArea[j]<FILTERS_Hybrid[2]){
									setResult("Fiber PhenoType", j, "IIA/IIX");
									AREA_Hybrid=AREA_Hybrid+FiberArea[j];
									nbFiberTypeHybrid++;
									nbFiberTyped++;
								}else{
									setResult("Fiber PhenoType", j, "NA");
		...
		Total_Fiber_Area=AREA_I+AREA_2A+AREA_2X+AREA_Not2X+AREA_2amb+AREA_Hybrid;
		...
		MnAREA_Hybrid=AREA_Hybrid/nbFiberTypeHybrid;
		if ((checkAnalysis[1]) || (checkAnalysis[3])){
			if (ik==0) {
				...
				if ((checkAnalysis[1]) && (checkAnalysis[3]))  {
					...
					TableResume[0]=TableResume[0]+"\tHybrid Type IIA/IIX Fibers\tHybrid Type IIA/IIX Total Area\tHybrid Type IIA/IIX Mean Area";
				}
				if ((checkAnalysis[1]) && (!checkAnalysis[3]))  {
					TableResume[0]=TableResume[0]+"\tType I Fibers\tType I Total Area\tType I Mean Area";
			...
			if ((checkAnalysis[1]) && (checkAnalysis[3]))  {
				...
				TableResume[nCurrentLine]=TableResume[nCurrentLine]+"\t"+nbFiberTypeHybrid+"\t"+AREA_Hybrid+"\t"+MnAREA_Hybrid;
			}
			if ((checkAnalysis[1]) && (!checkAnalysis[3]))  {
				TableResume[nCurrentLine]=TableResume[nCurrentLine]+"\t"+nbFiberType1+"\t"+AREA_I+"\t"+MnAREA_I;
...
if(checkAnalysis[1]){
	...
	if (FILTERS_I[0]){
		...
		File.append("Type I fibers were not filtered for size", RUN_INFO);
	}
}
if(checkAnalysis[3]){
	...
	if (FILTERS_IIA[0]){
		File.append("Type IIA fibers were filtered for size, with area Min: "+FILTERS_IIA[1]+" and Max "+FILTERS_IIA[2], RUN_INFO);
	}else{
		File.append("Type IIA fibers were not filtered for size", RUN_INFO);
	}
	...
	if (FILTERS_IIX[0]){
		File.append("Type IIX fibers were filtered for size, with area Min: "+FILTERS_IIX[1]+" and Max "+FILTERS_IIX[2], RUN_INFO);
	}else{
		File.append("Type IIX fibers were not filtered for size", RUN_INFO);
	}
	File.append("------Hybrid Type IIA/IIX-------", RUN_INFO);
	if(checkAnalysis[8]){
		File.append("Non-Type 1 fibers between lower and upper threshold classified as Hybrid Type IIA/IIX", RUN_INFO);
		if (FILTERS_Hybrid[0]){
			File.append("Hybrid Type IIA/IIX were filtered for size, with area Min: "+FILTERS_Hybrid[1]+" and Max "+FILTERS_Hybrid[2], RUN_INFO);
		}else{
			File.append("Hybrid Type IIA/IIX fibers were not filtered for size", RUN_INFO);
		}
	}else{
		File.append("Not included, Non-Type 1 fibers above lower threshold classified as Type IIA", RUN_INFO);
	}
}
close("*");
